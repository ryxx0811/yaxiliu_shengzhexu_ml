...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        other_direction=executor.submit(to_nearest_others,self_position,safe_others,field,explosion_map)
        crate_direction=executor.submit(to_nearest_crate,self_position,safe_crates,bombs,field,self_hasbomb)
        coin_direction=executor.submit(to_nearest_coin,self_position,safe_coins,field)
        safe_direction=executor.submit(to_nearest_safe_place,self_position,bombs,field,safe_places)
    ...
    new_safe_direction=to_nearest_safe_place(new_self_position,new_bombs,new_field,new_safe_place)
    if is_in_dangerous_area(self_position,bombs,field,explosion_map):
        ...
        if not is_in_dangerous_area(new_self_position,new_bombs,new_field,new_explosion_map):
            events.append(DANGEROUS_ZONE_TO_SAFE_ZONE)

    #
    else:
        if is_in_dangerous_area(new_self_position,new_bombs,new_field,new_explosion_map):
            if self_action!='BOMB':
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.model.transitions.append(Transition(state_to_features(last_game_state), last_action, np.zeros(626),reward_from_events(self, events)))
